...
const termDefinitions: Record<string, string> = {
  ...
  'P&C': 'Prinsip dan Kriteria - standar dan persyaratan yang harus dipenuhi untuk sertifikasi RSPO',
};
  ...
  return words.map((word, index) => {
    ...
    if (termDefinitions[cleanWord]) {
      return (
        <span key={index}>
          {cleanWord}
          <TermTooltip term={cleanWord} definition={termDefinitions[cleanWord]} />
          {punctuation}{' '}
        </span>
      );
    }
